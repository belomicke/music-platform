...
export const useUserFollowedUsers = (id: ComputedRef<string>) => {
    ...
    const mediaListId = computed(() => {
        return getUserFollowedUsersMediaListId.value(id.value)
    })
    ...
    const { fetch, isLoading } = useFetch(async () => {
        return await api.users.getFollowedUsers(id.value, users.value.length)
    }, {
        onSuccess: (res) => {
